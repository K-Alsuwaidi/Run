...
for target_size in range(9):
    ...
    for Iteration in range(20):
        ...
        history = FineTuneModel1.fit(trainData1, trainTarget1, batch_size=batch_size, verbose=verbose,
                                     epochs=nb_epoch)
        ...
        if (target_portion < 1470):
            ...
            refinment_model.fit(proxy_label_data, trainTarget1, batch_size=batch_size, verbose=verbose,
                                epochs=nb_epoch)
            ...
            for k in range(10, trainData1_unsupervisied1.shape[0], 10):
                ...
                history = UnSupModel1.fit(all_data, all_data_label, batch_size=batch_size, verbose=verbose,
                                          epochs=nb_epoch)
                LossIter.append(history.history.get("mse"))
                ...
                tRMSE4.append(x)
                kRMSE4.append(k)
                LossK.append(history.history.get("mse"))

                print("Target 1", env1, "K:", k, "fine tuning and psudolabel:", tRMSE4[-1])

                try:
                    if (tRMSE4[-1]>tRMSE[-2]) and (tRMSE[-2]>tRMSE[-3]):
                        break
...
np.save("Transfer_Learning_And_Semi-Supervised_Learning_256B_300e", Data)
